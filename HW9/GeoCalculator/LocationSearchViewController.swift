...
class LocationSearchViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        form = Section("Calculator Inputs") <<< LabelRow () { row in
            ...
            +++ Section("Calculation Date") <<< DateRow(){
                row in
                row.title = "Date"
                row.value = Date()
    ...
    func savePressed() {
        ...
        if errors.count > 0 {
            ...
            self.delegate?.set(calculationData: LocationLookup(origLat: p1Lat, origLng: p1Lng, destLat: p2Lat, destLng: p2Lng, timestamp: endDate))
            _ = self.navigationController?.popViewController(animated: true)
        }
    }
